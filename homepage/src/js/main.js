...
// Initialize intro path data
const pathEl = document.querySelector(".shape-wrap path");
if (pathEl) {
  pathEl.setAttribute('pathdata:intro', pathEl.getAttribute('d'));
}

function togglePage(direction) {
	if (window.isAnimating) return;
	// Prevent redundant actions
	if (direction === 'main' && window.currentPage === 'main') return;
	if (direction === 'intro' && window.currentPage === 'intro') return;
	...
	const isGoingToMain = direction === 'main';

	// Animation config
	const introY = isGoingToMain ? "-200vh" : "0vh";
	const targetPath = isGoingToMain 
		? DOM.path.getAttribute("pathdata:id") 
		: DOM.path.getAttribute("pathdata:intro");
...
function loadMain() {
	if (window.gridInstance) return;
	
	$(".card-inner").classList.add("in");
	setTimeout(() => {
		const canvas = document.getElementById("gridCanvas");
		// Create canvas if it was removed
		if (!canvas) {
			// This might need logic to recreate canvas element if it's completely gone from DOM
			// But for now let's assume it might still be there or we won't remove it fully
		}
		...
		if (canvas) {
			...
			gridAnimation.init();
			window.gridInstance = gridAnimation;
		}
	}, 500);
}
...
function unloadMain() {
	...
	$(".card-inner").classList.remove("in");
}
...
function handleScrollEvent(e) {
	...
	if (deltaY > 0) {
		// Scroll Down -> Go to Main
		togglePage('main');
	} else if (deltaY < 0) {
		// Scroll Up -> Go to Intro
		togglePage('intro');
	}
}
